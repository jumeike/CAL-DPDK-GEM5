...
CACHE_CONFIG="--caches --l2cache --l3cache --l3_size 4MB --ddio-enabled --l3_assoc 16 --l1i_size=64kB --l1i_assoc=4 \
...
if [[ -n "$checkpoint" ]]; then
  ...
  RUNDIR=${GIT_ROOT}/rundir/ckpts-with-new-vmlinux/memcached-kernel-O3CPU/$L2_SIZE"l2-"$FREQ"freq"-$PACKET_RATE"pkt-ddio-enabled"
  setup_dirs
  CPUTYPE="DerivO3CPU" # just because DerivO3CPU is too slow sometimes
  GEM5TYPE="opt"
  LOADGENREPLAYMODE=${LOADGENREPLAYMODE:-"ConstThroughput"}
  DEBUG_FLAGS="--debug-flags=EthernetAll" #LoadgenDebug"
  CONFIGARGS="--l2_size=$L2_SIZE --cpu-clock=$FREQ $CACHE_CONFIG $CPU_CONFIG -r 2 --loadgen-type=Pcap --loadgen_pcap_filename=$PCAP_FILENAME --loadgen-start=20619488205000 -m=23619488205000 --packet-rate=$PACKET_RATE --loadgen-replymode=$LOADGENREPLAYMODE --loadgen-port-filter=$PORT --loadgen-increment-interva=$INCR_INTERVAL"
